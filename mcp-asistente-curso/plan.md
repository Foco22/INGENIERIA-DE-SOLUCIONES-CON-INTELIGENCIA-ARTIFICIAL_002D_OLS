...
5. **Grafo de conocimiento**: pase de extracción con LLM sobre el `.md` de cada PDF → entidades (conceptos, frameworks, técnicas) y relaciones entre ellas. Merge incremental en `graph.json` (`networkx`, serializado) — se guarda **después de cada PDF**, no solo al final, para no perder progreso si se corta a mitad de camino. LLM: **Groq**, `llama-3.3-70b-versatile` (probado contra `llama-3.1-8b-instant`: el chico saca buenas entidades pero relaciones con errores — ej. "GPT-3 es parte de LangGraph" — así que se descartó pese a ser más rápido). Corre solo en el pipeline de indexación, no por pregunta de estudiante.
   > **Límite real encontrado:** el tier gratuito de Groq tiene tope de **100,000 tokens/día por modelo** (no por minuto) — sacarle entidades/relaciones a los 27 PDFs completos de una sentada no alcanza, hay que hacerlo en varias tandas a lo largo del día/días. Por eso el guardado incremental + reintento diff-aware (retoma solo lo que falta) — ver `ESTADO.md` para el progreso actual del bootstrap.
